...
User chọn Game → Chọn Chế độ tạo số → Bấm "Tạo bộ số"
    ...
    │                   │                   │
┌───▼───┐  ┌────▼────┐  ┌───▼────┐  ┌───▼────┐  ┌───▼────┐
│ Ngẫu  │  │ Thông   │  │ Dự     │  │ Nâng   │  │ Cân    │
│ nhiên │  │ minh    │  │ đoán   │  │ cao    │  │ bằng   │
└───┬───┘  └────┬────┘  └───┬────┘  └───┬────┘  └───┬────┘
    │           │           │           │           │
    ▼           ▼           ▼           ▼           ▼
Fisher-Yates  Loại trừ   Phân tích  Random.org  Odd/Even
Shuffle       số xấu     lịch sử    +Entropy   High/Low
    │           │           │           │       Sum Filter
    └───────────┴─────┬─────┴───────────┘           │
                      ▼                             │
              Hiển thị kết quả◄─────────────────────┘
...
### Chiến lược tạo số (strategy)

| Strategy | Mô tả | Đặc điểm |
|----------|-------|----------|
| `random` | Ngẫu nhiên thuần túy | Fisher-Yates shuffle |
| `smart` | Thông minh | Loại trừ số xấu, kém may mắn |
| `prediction` | Dự đoán | Dựa trên tần suất + gap lịch sử |
| `enhanced` | Nâng cao | Random.org API + User entropy |
| `balanced` | **Cân bằng (MỚI)** | Odd/Even, High/Low, Sum filter |

#### Chi tiết strategy=balanced:
- **Odd/Even balance**: 2-4 số chẵn, 2-4 số lẻ
- **High/Low balance**: 2-4 số cao (23-45), 2-4 số thấp (1-22)
- **Sum filtering**: Tổng nằm trong khoảng 100-190
- **Pattern exclusion**: Loại các pattern hiếm (>3 số liên tiếp, cùng decade)
